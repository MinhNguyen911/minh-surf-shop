...
module.exports = {
	...
	async putForgotPw(req,res,next){
		...
		if (!user){
			req.session.error = 'No account with that email registered!';
		...
		const msg = {
			...
			requested the reset of the password for your account. Please click
			on the following link, or copy and paste it into your browser to
			complete the process: http://${req.headers.host}/reset/${token}
			If you did not request this, please ignore this email and your
			password will remain unchanged.`.replace(/			/g,''),
		};
	...
	async putReset(req,res, next){
		...
		if(req.body.password === req.body.confirm){
			...
			await user.save();
		//log the user in now
			const login = util.promisify(req.login.bind(req)); //return a promise here
			await login(user);
		} else{
		...
		const msg = {
			to: user.email,
			from: 'Surf Shop Admin <mhn3@sfu.ca>',
